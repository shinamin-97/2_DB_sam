...
SELECT DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별,
			...
			SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여' ) -- 별칭 사용X(SELECT절 해석 X)
...
-- EMPLOYEE 테이블에서 직급 별 2000년도(2000년도 포함) 이후 입사자들의 급여 합을 조회
-- (직급코드 오름차순)
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
WHERE HIRE_DATE >= '2000-01-01'
-- WHERE EXTRACT( YERA FROM HIRE_DATE ) >= 2000
GROUP BY JOB_CODE
ORDER BY 1;
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPY_CODE로 그룹을 나누고,
...
-- EMPLOYEE 테이블에서 직급별 인원 수가 5명 이하인 직급코드, 인원 수 조회(직급코드 오름차순)
...
-- CUBE : GROUP BY 절에서 작성된 모든 컬럼의 중간 집계를 처리하는 함수
...
/* SET OPERATOR (집합 연산자)
 ...
 - UINON (합집합) : 두 SELECT 결과를 하나로 합침
 					단, 중복은 한 번만 작성
 
 - INTERSECT (교집합) : 두 SELCCT 결과 중 중복되는 부분만 조회
...
-- 서로 다른 테이블이지만 커럼의 타입, 개수만 일치하면 집합 연산자 사용 가능!
