 ...
 *   이떄, 조회된 행들의 집합을 "RESULT SET" (조회 결과의 집합) 이라고 한다.  
 * 
...
--<컬럼 값 산술 연산>
-- 컬럽 값 : 테이블 내 한 칸(==한 셀)에 작성된 값(DATE)
...
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE NOT SALARY BETWEEN 3000000 AND 6000000;

/* 날짜(DATE)에 BETWEEN 이용하기 */
-- EMPPLOYEE 테이블에서 입사일이 1990-01-01 ~ 1999-12-31 사이인 직원의
-- 이름, 입사일 조회
...
-- 해경 방법 : LIKE의 ESCAPE OPTION을 이용하DU _를 구분한다.
...
-- 부서코드가 'D9' 또는 'D6'이고 -> AND 가 OR보다 우선순위가 높다.
 ...
 * 1. FROM절
 * 2. WHERE절
 * 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC] [NULLS FIRST|LAST] []<= 안에 있는 것들은 생략 가능
 * 										(오름/내림 차순)
 */
