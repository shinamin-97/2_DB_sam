...
CREATE USER sam_sample IDENTIFIED BY sample1234;

-- 2. 새 연결(접속 방법) 추가
--> 오류 : 접속 권한이 없어서 로그인 실패

--3. (SYS) 접속 권한 부여

-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권한,.... TO 사용자명
GRANT CREATE SESSION TO sam_sample;
...
GRANT CREATE TABLE TO sam_sample;

ALTER USER sam_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO sam_sample;
...
-- kh_sam  /  sam_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh_sam 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_sam.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다.
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) sam_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO sam_sample;

-- 3. (sample) 다시 kh_sam.EMPLOYEE 조회
SELECT * FROM kh_sam.EMPLOYEE;
...
-- 4. (sample) kh_sam.EMPLOYEE 테이블을 복사한 테이블 생성
CREATE TABLE EMP_SAMPLE
AS SELECT * FROM kh_sam.EMPLOYEE;
...
REVOKE SELECT ON EMPLOYEE FROM sam_sample;
