/*
    * SUBQUERY (서브쿼리)
    - 하나의 SQL문 안에 포함된 또다른 SQL(SELECET)문
...
-- 3) 전 직원의 평균 급여보다 많은(이상) 급여를 받고 있는 직원 조회
...
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE /*3*/
FROM EMPLOYEE MAIN
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
WHERE ENT_YN = 'N'
...
-- 1) MAIN 쿼리의 1행의 DEPT_CODE를 서브쿼리에 대입
-- 2) SUB를 수행
