...
-- 함수는 SELECT문의
-- SELECT절, WHERE절, ORDER BY, GROUP BY절, HAVING절, 사용 가능
...
-- 문자열을 5번째 문자 부터 검색하여 첫 번쨰 B위치 조회
...
-- EMPLOYEE 테이브블에서 사원명, 이메일, 이메일 중 '@('at') 위치 조회
...
SELECT ROUND(123.456, 1) FROM DUAL; -- 123.5, 소수점 두 번쨰 자리에서 반올림
									--> 두 번째 자리에서 반올림 해서 
									--> 소수점 한 자리만 표현

SELECT ROUND(123.456, 0) FROM DUAL; -- 소수점 첫 번쨰 자리에서 반올림(0 기본값)

SELECT ROUND(123.456, -1) FROM DUAL; -- 소수점 0 번쨰 자리에서 반올림
									 --> 소수점 -1 자리 표현
...
SELECT TO_NUMBER'1,000,000' + 500,000 FROM DUAL;
-- 수치가 부적합
...
-- EMPLOYEE 테이블에서
-- 급여가 500만원 이상이면 '대'
-- 급여가 300만 이상 500만원 미만이면 '중'
-- 급여가 300만 미만이면 '소' 으로 조회
...
-- 급여 최대값, 가장 느린 입사일, 알파벳 순서가 가장 느린 이메일
...
--  COUNT(칼럼명)에 의해 NULL을 제외한 실제 값이 있는 행의 개수만 조회
